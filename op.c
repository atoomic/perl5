...
S_pmtrans(pTHX_ OP *o, OP *expr, OP *repl)
{
    ...
    /* khw doesn't know how these could differ in UTF-8ness, since both sides
     * come from the same tr/// */
...
do_trans_simple_utf8() into one.

These are capable of handling both UTF-8 and non-UTF-8 cases, but I left in the
existing non-UTF-8 implementation, which uses tables, because I suspect it is
faster.  This means that there is extra code, purely for runtime performance.
